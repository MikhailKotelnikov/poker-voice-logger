...
`Poker Voice Logger` — это локальный рабочий стол для разбора раздач и накопления игровых наблюдений в единой структуре.

Ключевая идея:
- voice-ноты и HH-ноты приводятся к одному каноничному формату street-полей,
- анализ и визуализация строятся поверх локальной HH БД,
- manual-наблюдения (пресуппозиции и тайминги действий) живут отдельно от raw HH и не теряются при очистке/реимпорте рук.

Практический сценарий:
1. Во время сессии пользователь быстро диктует или вручную дополняет заметки.
2. После сессии загружает HH пачками (или через автоимпорт папки).
3. Открывает профиль, фиксирует дополнительные чтения по конкретным действиям (тайминги, локальные пресуппозиции, hand-presup).
4. При пересборке HH-данных manual-наблюдения автоматически ре-линкуются по ключу раздачи.
...
- Кнопка `Стереть всю HH базу` в главном окне приложения (с confirm).
...
- Кнопка `профиль DB` под каждым оппом: открывает визуализацию по HH SQLite данным.
...
- В list-режиме визуализации HH доступны inline-аннотации:
  - локальные пресуппозиции по улицам (`preflop/flop/turn/river`),
  - общее поле `presup` для всей раздачи,
  - ручной тайминг для каждого действия (`% t` и `% tb`).
- Ввод для street/hand пресуппозиций:
  - текстом (сохранение по `Enter`),
  - голосом через inline-микрофон в поле.
- `Сохранить репорт` для manual audio-правок доступен на уровне раздачи (в строке meta).
- В модалке HH профиля доступно `Стереть руки игрока` (с confirm) — удаляет только HH-структуру по выбранному target.
...
- Manual-наблюдения для HH (street/hand-presup + action timing) хранятся в отдельных таблицах и привязываются по ключу:
  - `target_identity + room + hand_number` (+ `street + action_index` для timing).
...
- Для HH manual audio-режима репорт сохраняется на уровне раздачи и последнего озвученного поля.
...
- Tooltip из графика поддерживает режим pin:
  - hover -> временный показ,
  - single click -> закрепление для интерактивного редактирования,
  - double click -> закрытие закрепленного popup.
...
- `GET /api/opponent-visual-list?opponent=...&source=all|voice|hh` (list-режим раздач для интерактивной правки)
...
- `POST /api/hh-manual-presupp-text` (ручное сохранение HH пресуппозиции по раздаче)
- `POST /api/hh-manual-presupp-audio` (голосовое сохранение HH пресуппозиции)
- `POST /api/hh-manual-timing-text` (ручной тайминг действия в HH)
- `POST /api/hh-clear-opponent` (очистка HH рук выбранного игрока)
- `POST /api/hh-clear-all` (очистка всех HH рук без удаления manual-наблюдений)
...
- Заголовок `Omaha Pot Limit` без явного `N Card Omaha` трактуется как `PLO4`.
- Префлоп сайзинги -> `bb`.
- Постфлоп ставки/рейзы -> `%pot`.
- Лимит берется из хедера `(<SB>/<BB> CNY)` и хранится как `SB-BB`; валютный префикс (`¥`) не влияет на вычисления BB/%pot.
- В showdown-спотах:
...
- Interactive tooltip из графика должен поддерживать pin/unpin, иначе редактирование manual-полей и timing невозможно.
